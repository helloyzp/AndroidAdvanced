 ...
 * View，就是添加到界面中的视图；固有属性，就是Attr指得是Fragment的一些固有属性，不会随着Fragment的生命周期发生变化；
 * State，就是Fragemnt当前的状态，Fragment在生命周期变迁中中会发生改变的状态。其实说白了，Fragment就是一个拥有生命周期的自定义View
 */
...
//3		STOPPED				Fragment已经被创建，但是尚未不可见							dispatchStop 
    ...
    //Fragmemt 绑定的对象，一般就是 Activity 和 Fragment
    ...
    //当前 Fragment 的布局
    //在 onCreateView() 中返回一个 布局，
    //然后在 FragmentManager 中拿到这个布局，
    //添加到要绑定容器（Activity/Fragment）的 ViewGroup 中，然后设置相应的状态值
    View mView;
    ...
    final void restoreViewState(Bundle savedInstanceState) {
        ...
        if (!mCalled) {
            throw new androidx.fragment.app.SuperNotCalledException("Fragment " + this
