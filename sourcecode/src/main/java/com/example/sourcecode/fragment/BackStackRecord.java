 ...
 * 每次对Fragment进行提交操作，都会生产一个BackStackRecord对象。然后，将对Fragment的每一步操作都封装成Op对象，添加到BackStackRecord中去。
 * BackStackRecord实现了FragmentManagerImpl.OpGenerator 接口，当generateOps的时候，如果需要添加到回退栈
 * 则可以被addBackStackState保存回退栈状态，表示BackStackRecord可以被FragmentManager保存到操作堆栈中，
 * 到执行popBackStack()时，可以恢复原先的BackStackRecord事务
 */
final class BackStackRecord extends FragmentTransaction implements
    ...
    /**
     * Op对象，主要保存了FragmentTransaction操作事务中的单次操作，是相对于Fragment而言的。每个Op对象，保存了操作的Fragment对象，切换动画等对象
     */
    ...
    public boolean generateOps(ArrayList<BackStackRecord> records, ArrayList<Boolean> isRecordPop) {
        ...
        if (mAddToBackStack) {
            mManager.addBackStackState(this);//保存回退栈状态
        }
        return true;
    }
    ...
    void executePopOps(boolean moveToState) {
        ...
        if (!mReorderingAllowed && moveToState) {
            mManager.moveToState(mManager.mCurState, true); //遍历完该事务后执行moveToState()对mAdded 和mActive 中的fragment切换生命周期。
        }
    }
